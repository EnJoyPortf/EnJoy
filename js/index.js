...
// init Isotope
var $gridF = $('.gridPortfolio').isotope({
	itemSelector : '.img-container',
// layoutMode: 'fitRows',
});

var $gridV = $('.gridVideos').isotope({
	itemSelector : '.img-container',
// layoutMode: 'fitRows',
});
...
//bind filter button click
$('#filter-btnV').on('click', 'button', function() {
	var filterValue = $(this).attr('data-filter');
	// use filterFn if matches value
...
$(document).ready(
		function() {
			...
			var about_scene = new ScrollMagic.Scene({
				...
				triggerHook : .7
			}).setClassToggle('.about-text', 'about-text-animate').reverse(
					false).addTo(controller2);
			...
			var about_scene = new ScrollMagic.Scene({
				...
				triggerHook : .7
			}).setClassToggle('.icon', 'service-icon-animate').reverse(false)
					.addTo(controller2);
